...
export default function Tenants({ tenants = [], rooms = [], ledger = [], properties = [], setSecurityModal, setPaymentModal, setEditResidentModal, calculateBalanceAtPeriod, currentPeriodLabel }) {
              ...
              <div className="relative w-full max-w-md bg-white h-full shadow-2xl p-10 flex flex-col animate-in slide-in-from-right duration-300">
                  <div className="flex justify-between items-start mb-10">
                    <div>
                      ...
                      <p className="text-indigo-500 font-bold uppercase text-xs italic tracking-widest mt-1">{(properties.find(p=>p.id===selectedTenant.property_id)?.name) || '---'} • Room {(rooms.find(r => r.id === selectedTenant.room_id)?.room_number) || 'NA'}</p>
                    </div>
                    ...
                    {showHistory ? (
                        <div className="space-y-4">
                            ...
                            {(ledger || []).filter(l => l.tenant_id === selectedTenant.id).map(log => (
                                <div key={log.id} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center border shadow-sm">
                                    ...
                                    <div className={`text-[9px] font-black uppercase px-3 py-1 rounded-xl border ${log.payment_type?.toUpperCase() === 'RENT' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>{log.payment_type}</div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <>
                            <div className="p-8 bg-[#1E1E45] rounded-[2.5rem] text-white shadow-2xl space-y-4 shadow-xl">
                                <div className="flex justify-between border-b border-white/10 pb-4"><p className="opacity-60 text-[10px] font-black uppercase tracking-widest italic">Current Due</p><p className="text-2xl font-black italic text-rose-400">₹{calculateBalanceAtPeriod(selectedTenant, ledger, currentPeriodLabel).toLocaleString()}</p></div>
                                <div className="flex justify-between pt-2"><p className="opacity-60 text-[10px] font-black uppercase tracking-widest italic">Agreement Rent</p><p className="text-xl font-bold italic tracking-tighter">₹{selectedTenant.agreed_rent?.toLocaleString()}</p></div>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                <button onClick={() => window.open(`tel:${selectedTenant.phone_number}`)} className="p-4 bg-slate-50 rounded-2xl flex items-center justify-center gap-3 font-black uppercase text-[10px] shadow-sm hover:bg-indigo-600 hover:text-white transition-all"><Phone size={14}/> Call</button>
                                <button onClick={() => window.open(`https://wa.me/91${selectedTenant.phone_number}`)} className="p-4 bg-emerald-50 rounded-2xl flex items-center justify-center gap-3 font-black uppercase text-[10px] shadow-sm hover:bg-[#25D366] hover:text-white transition-all text-[#25D366] border border-emerald-100"><MessageCircle size={14}/> WhatsApp</button>
                            </div>
                            <div className="bg-slate-50 p-8 rounded-[2.5rem] space-y-6 border border-slate-100 shadow-inner">
                                <div className="space-y-4">
                                    <div className="flex items-start gap-4"><Calendar className="text-indigo-500 mt-1" size={18}/><p className="text-sm font-bold">Joined: {selectedTenant.join_date}</p></div>
                                    <div className="flex items-start gap-4"><ShieldCheck className="text-emerald-500 mt-1" size={18}/><p className="text-sm font-bold uppercase">ID: {selectedTenant.aadhaar_number || '---'}</p></div>
                                    <div className="flex items-start gap-4"><Briefcase className="text-indigo-500 mt-1" size={18}/><p className="text-sm font-bold italic leading-tight">Work: {selectedTenant.organization_name || 'Individual'}</p></div>
                                    <div className="flex items-start gap-4"><MapPin className="text-rose-500 mt-1" size={18}/><p className="text-sm font-bold text-slate-500 italic leading-tight">{selectedTenant.permanent_address || '---'}</p></div>
                                    <div className="flex items-start gap-4"><PhoneCall className="text-rose-500 mt-1" size={18}/><p className="text-sm font-bold">Emergency: {selectedTenant.emergency_number || '---'}</p></div>
                                </div>
                            </div>
