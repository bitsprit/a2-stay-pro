...
export default function App() {
  ...
  const [editResidentModal, setEditResidentModal] = useState({ open: false, tenant: null });
  ...
  const calculateBalanceAtPeriod = (tenant, allLedger, targetPeriodLabel) => {
    ...
    try {
      ...
      const monthsStayed = (tY - joinDate.getFullYear()) * 12 + (tM - joinDate.getMonth()) + 1;
      let totalCharge = monthsStayed * Number(tenant.agreed_rent);
      const totalPaid = (allLedger || []).filter(l => l.tenant_id === tenant.id && l.is_verified && l.payment_type?.toUpperCase() === 'RENT').reduce((sum, curr) => sum + (Number(curr.paid_amount) || 0), 0);
      const bal = totalCharge - totalPaid;
      const now = new Date();
      if (tM === now.getMonth() + 1 && tY === now.getFullYear() && now.getDate() <= 5 && bal <= Number(tenant.agreed_rent)) return 0;
      return bal;
    } catch (e) { return 0; }
  };
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (new Date(selY, selM - 1, 1) <= new Date()) {
 ...
 const handleBedAction = async (room, action, extra = null) => {
    ...
    else if (action === 'book') up = { 
      booked_beds: 1, 
      booked_by_name: extra.name, 
      booked_by_phone: extra.phone, 
      advance_amount: Number(extra.amount), 
      booking_date: extra.date 
    };
    else if (action === 'cancel') up = { 
      booked_beds: 0, 
      booked_by_name: null, 
    ...
    const { error } = await supabase.from('rooms').update(up).eq('id', room.id);
  ...
  const handleFinalCheckIn = async (e) => {
    ...
    const { data: nt, error: te } = await supabase.from('tenants').insert([{ 
      ...
      security_deposit: Number(f.get('t_security')), join_date: f.get('t_join_date') 
    }]).select().single();
    if (!te && nt) {
        await supabase.from('ledger').insert([
          { tenant_id: nt.id, property_id: selectedProperty.id, billing_month: getPeriodLabel(currentPeriod), paid_amount: Number(f.get('t_rent')), is_verified: true, payment_type: 'RENT', payment_mode: 'CASH' },
          { tenant_id: nt.id, property_id: selectedProperty.id, billing_month: getPeriodLabel(currentPeriod), paid_amount: Number(f.get('t_security')), is_verified: true, payment_type: 'SECURITY', payment_mode: 'CASH' }
        ]);
  ...
  const handleUpdateResident = async (e) => {
    ...
    }).eq('id', editResidentModal.tenant.id);
    setEditResidentModal({ open: false, tenant: null }); fetchData();
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans text-slate-800 selection:bg-indigo-100">
      <aside className="w-full md:w-64 bg-[#1E1E45] text-white p-6 flex flex-col z-50 shadow-2xl">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <button onClick={() => setView('tenants')} className={`w-full text-left px-4 py-4 rounded-2xl font-bold text-xs transition-all ${view === 'tenants' ? 'bg-white/10 shadow-inner' : 'opacity-40 hover:opacity-100'}`}><Users size={18} className="inline mr-4"/> Residents</button>
        </nav>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto no-scrollbar">
          ...
          <div className="animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-10">
                ...
                <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border shadow-sm">
                    ...
                    <button onClick={() => setAddPropModal(true)} className="ml-2 bg-slate-900 text-white p-3 rounded-xl shadow-lg hover:scale-105 transition-all"><Plus size={18}/></button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
              ...
              <button onClick={() => setFinanceDetail({ open: true, type: 'Security', data: ledger.filter(l => l.billing_month === getPeriodLabel(currentPeriod) && l.payment_type?.toUpperCase() === 'SECURITY') })} className="bg-white border-2 p-8 rounded-[2.5rem] text-left shadow-sm active:scale-95 transition-all"><p className="text-[10px] font-black text-slate-400 uppercase mb-1 italic">Security</p><h2 className="text-3xl font-black italic">₹{stats.security.toLocaleString()}</h2></button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {properties.map(p => (
                <div key={p.id} className="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-sm relative group border-b-8 border-slate-900 transition-all hover:shadow-2xl">
                  <button onClick={(e) => { e.stopPropagation(); setSecurityModal({ open: true, type: 'property', id: p.id }); }} className="absolute top-8 right-8 p-3 text-rose-400 opacity-0 group-hover:opacity-100 hover:bg-rose-50 rounded-full transition-all"><Trash2 size={20}/></button>
                  ...
                  <button onClick={() => { setSelectedProperty(p); setView('inventory'); }} className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Manage Building</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {view === 'inventory' && <Inventory selectedProperty={selectedProperty} rooms={rooms} tenants={tenants} ledger={ledger} properties={properties} handleBedAction={handleBedAction} setCheckInModal={setCheckInModal} setPaymentModal={setPaymentModal} setSecurityModal={setSecurityModal} setBookingModal={setBookingModal} setAddRoomModal={setAddRoomModal} setEditResidentModal={setEditResidentModal} setView={setView} calculateBalanceAtPeriod={calculateBalanceAtPeriod} currentPeriodLabel={getPeriodLabel(currentPeriod)} />}
        {view === 'tenants' && <Tenants tenants={tenants} rooms={rooms} ledger={ledger} properties={properties} setSecurityModal={setSecurityModal} setPaymentModal={setPaymentModal} setEditResidentModal={setEditResidentModal} calculateBalanceAtPeriod={calculateBalanceAtPeriod} currentPeriodLabel={getPeriodLabel(currentPeriod)} />}
      </main>

      {/* MODALS */}
      {addPropModal && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[800] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-md shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={async (e) => { 
              ...
              <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase shadow-xl italic transition-all hover:bg-black">Generate</button>
      ...
      {addRoomModal && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[900] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-md shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={async (e) => { 
              ...
              <input name="r_num" placeholder="Room Number" required className="w-full bg-slate-50 p-5 rounded-2xl font-bold border" />
              <select name="r_type" className="w-full bg-slate-50 p-5 rounded-2xl font-bold border"><option value="Single">Single Bed</option><option value="Double">Double Bed</option><option value="Triple">Triple Bed</option></select>
              <button type="submit" className="w-full py-5 bg-emerald-500 text-white rounded-2xl font-black uppercase shadow-xl italic transition-all active:scale-95">Confirm Room</button>
      ...
      {checkInModal.open && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[900] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-4xl shadow-2xl overflow-y-auto max-h-[90vh] animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-10"><h3 className="text-3xl font-black italic uppercase text-[#1E1E45]">Register Resident</h3><button onClick={() => setCheckInModal({ open: false, room: null })}><X size={24}/></button></div>
                <form onSubmit={handleFinalCheckIn} className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4"><input name="t_phone" required className="w-full bg-slate-50 p-5 rounded-2xl font-black outline-none shadow-inner border" placeholder="Mobile" /><input name="t_aadhaar" required className="w-full bg-slate-50 p-5 rounded-bold border shadow-inner" placeholder="Aadhaar" /></div>
                    ...
                    <div className="space-y-6 pt-12">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-2 italic font-bold text-rose-500">Monthly Rent</label><input name="t_rent" type="number" required className="w-full bg-rose-50 p-5 rounded-2xl font-black text-xl outline-none" /></div>
                            <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-2 italic font-bold text-indigo-500">Security</label><input name="t_security" type="number" required className="w-full bg-indigo-50 p-5 rounded-2xl font-black text-xl outline-none" /></div>
                        </div>
                        ...
                        <button onClick={() => setCheckInModal({ open: false, room: null })} type="button" className="w-full text-slate-400 font-bold uppercase text-[10px] text-center mt-2">Discard</button>
                    </div>
                </form>
            </div>
        </div>
      ...
      {editResidentModal.open && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[950] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white rounded-[3rem] p-12 w-full max-w-4xl shadow-2xl overflow-y-auto max-h-[90vh] animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-10"><h3 className="text-3xl font-black uppercase italic">Edit Profile</h3><button onClick={() => setEditResidentModal({ open: false, tenant: null })} className="hover:rotate-90 transition-all"><X size={24}/></button></div>
            <form onSubmit={handleUpdateResident} className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                <input name="e_name" defaultValue={editResidentModal.tenant?.full_name} placeholder="Full Name" required className="w-full bg-slate-50 p-5 rounded-2xl font-bold shadow-inner border" />
                <input name="e_phone" defaultValue={editResidentModal.tenant?.phone_number} placeholder="Mobile" required className="w-full bg-slate-50 p-5 rounded-2xl font-bold shadow-inner border" />
                <input name="e_aadhaar" defaultValue={editResidentModal.tenant?.aadhaar_number} placeholder="Aadhaar No" className="w-full bg-slate-50 p-5 rounded-2xl font-bold shadow-inner border" />
                <input name="e_emergency" defaultValue={editResidentModal.tenant?.emergency_number} placeholder="Emergency Phone" className="w-full bg-slate-50 p-5 rounded-2xl font-bold shadow-inner border" />
              </div>
              <div className="space-y-4">
                <input name="e_org" defaultValue={editResidentModal.tenant?.organization_name} placeholder="Office/College" className="w-full bg-slate-50 p-5 rounded-2xl font-bold border shadow-inner" />
                <div className="grid grid-cols-2 gap-4">
                  <input name="e_rent" type="number" defaultValue={editResidentModal.tenant?.agreed_rent} placeholder="Monthly Rent" required className="bg-slate-50 p-5 rounded-2xl font-bold border shadow-inner" />
                  <input name="e_security" type="number" defaultValue={editResidentModal.tenant?.security_deposit} placeholder="Deposit Stored" required className="bg-slate-50 p-5 rounded-2xl font-bold border shadow-inner" />
                </div>
                <textarea name="e_address" defaultValue={editResidentModal.tenant?.permanent_address} placeholder="Home Address" className="w-full bg-slate-50 p-5 rounded-2xl font-bold h-28 shadow-inner border" />
              </div>
      ...
      {paymentModal.open && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[950] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-md shadow-2xl animate-in zoom-in-95">
            <h3 className="text-2xl font-black uppercase mb-6 italic text-[#1E1E45] text-center">Record Receipt</h3>
            <p className="text-slate-400 text-xs font-bold uppercase mb-8 italic text-center">Resident: {paymentModal.tenant?.full_name}</p>
            <form onSubmit={async (e) => { 
                e.preventDefault(); const f = new FormData(e.target); 
                ...
                setPaymentModal({ open: false, tenant: null }); fetchData();
            }} className="space-y-6">
      ...
      {securityModal.open && (
        <div className="fixed inset-0 bg-[#1E1E45]/95 z-[999] flex items-center justify-center p-4 backdrop-blur-xl">
          <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-md shadow-2xl text-center animate-in zoom-in-95">
            <h3 className="text-2xl font-black uppercase italic mb-8 tracking-tighter font-serif">Verification Required</h3>
            ...
            <div className="flex gap-4"><button onClick={() => setSecurityModal({ open: false, type: null, id: null })} className="flex-1 py-5 rounded-2xl font-black uppercase text-slate-400 bg-slate-100 transition-all hover:bg-slate-200">Cancel</button><button onClick={() => executeSecureDelete(document.getElementById('admin_pin_input').value)} className="flex-1 py-5 rounded-2xl font-black uppercase bg-rose-600 text-white shadow-xl transition-all hover:bg-rose-700">Confirm Delete</button></div>
          </div>
        </div>
      )}
    </div>
