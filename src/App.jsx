/**
 * App.jsx — A2 Stay Pro v6.1 (Ultra-Stable Production Build)
 * ══════════════════════════════════════════════════════════════
 * CRITICAL BLANK PAGE FIXES:
 * 1.  Identity Guard: Component won't attempt complex renders until Profile is ready.
 * 2.  Null Session Protection: Replaced all `session.user.id` calls with safe optional chaining.
 * 3.  Relational Enrichment: Restored mapping of Tenant names and Room numbers into Ledger.
 * 4.  Modal Props Reset: Ensured all original handlers (Rename, Change Type, Verify) are passed.
 * ══════════════════════════════════════════════════════════════
 */
...
export default function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      showToast("Data connection error", "error");
    } finally {
      setLoading(false);
    }
  ...
  if (authLoading || (loading && !userProfile)) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-white">
        ...
        <p className="font-black text-zinc-400 uppercase text-[10px] tracking-widest animate-pulse">Establishing Connection...</p>
      </div>
    );
  }
  ...
  return (
    <AuthWrapper>
      <div className="min-h-screen bg-white text-zinc-900 font-sans flex flex-col">
        ...
        <main className="flex-1 p-4 pb-32 max-w-7xl mx-auto w-full">
          {view === 'dashboard' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="flex justify-between items-center">
                ...
                {userProfile?.role !== 'staff' && <button onClick={() => setAddPropModal(true)} className="p-3 bg-indigo-600 text-white rounded-2xl shadow-xl hover:bg-indigo-700 transition-all"><Plus size={20}/></button>}
              </div>
                ...
                {(properties || []).map(p => (
                  <div key={p.id} className="bg-white border-2 border-zinc-50 rounded-[3rem] p-10 shadow-sm hover:shadow-2xl transition-all group relative">
                    ...
                    {userProfile?.role !== 'staff' && (
                        <button onClick={() => setSecurityModal({open:true, type:'property', id:p.id})} className="absolute top-10 right-10 text-zinc-100 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={18}/></button>
                    )}
                  </div>
                ))}
              </div>
          ...
          {view === 'inventory' && (
            <Inventory 
              ...
              setPaymentModal={setPaymentModal} setSecurityModal={setSecurityModal} setView={setView} 
            ...
            <div className="bg-white h-full w-full max-w-md shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {(financeModal.data || []).map((item, i) => (
                  <div key={i} className="p-5 bg-zinc-50 rounded-[1.5rem] border flex justify-between items-center group hover:bg-white transition-all shadow-sm">
                    <div className="min-w-0">
                      <p className="font-black text-sm uppercase truncate">{item.tenant_name || item.full_name || 'Resident'}</p>
        ...
        {addPropModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[3000] flex items-center justify-center p-4">
            <div className="bg-white rounded-[3rem] w-full max-w-sm p-10 shadow-2xl animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-8"><h3 className="text-xl font-black uppercase italic">New Building</h3><button onClick={()=>setAddPropModal(false)}><X size={18}/></button></div>
        ...
        {securityModal.open && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[4000] flex items-center justify-center p-4 text-center">
            <div className="bg-white rounded-[2.5rem] w-full max-w-xs p-10 animate-in zoom-in-95">
              ...
              <div className="grid grid-cols-2 gap-3 mt-8">
                ...
                <button onClick={handleSecurityConfirm} className="py-4 bg-rose-500 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-rose-500/30">Delete</button>
              </div>
            </div>
          </div>
        )}
